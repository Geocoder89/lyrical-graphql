...
const songMutations: MutationResolvers = {
  addSong: async (_: {}, { title}: MutationAddSongArgs) => {
    try {
  ...
  addLyricToSong: async (
    _parent: {},
    { songId, content }: MutationAddLyricToSongArgs
  ) => {
    try {
  ...
  deleteSong: async (_parent: {}, { id }: MutationDeleteSongArgs) => {
